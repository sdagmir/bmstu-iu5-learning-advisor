...
class ChatService:
    ...
    def _build_messages(
        ...
        # В контекст LLM попадают последние 5 сообщений диалога: 2 пары
        # вопрос-ответ из истории + текущий вопрос пользователя (добавляется
        # ниже). Этого хватает для уточняющих вопросов; больше — раздувает
        # контекст и стоимость без пользы.
        if history:
